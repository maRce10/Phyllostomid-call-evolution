...
# anadir campo sobre cuales selecciones hay q corregir
sls$correg.caja <- sapply(1:nrow(sls), function(x) {
  
  bd$Corregir.caja[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]
})

# revisar cuentas por categoria de corregir
table(sls$correg.caja)

# renombrar selecciones
sls$old.selec <- sls$selec
...
unique(sls$sound.files[sls$check.res == "sound file not found"])
unique(sls$sound.files[sls$check.res != "OK"])

# arreglar nombre de archivo tomandolo de la base de datos de excel
sls$sound.files[sls$check.res == "sound file not found"] <- sapply(which(sls$check.res == "sound file not found"), function(x) {
  
  
  
  
  bd$sound.files[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]})

# quitar los dedazos en las seleccions
sls <- sls[sls$check.res %in% c("OK", "sound file not found"), ] 
...
# arreglar nombre de archivo tomandolo de la base de datos de excel
sls$especie <- sapply(1:nrow(sls), function(x) {
  
  bd$Spp[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]
})


sls$sound.files <- sapply(1:nrow(sls), function(x) {
  
  bd$sound.files[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]
})

sls$fuente <- sapply(1:nrow(sls), function(x) {
  
  bd$Fuente[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]
})

sls$expansion <- sapply(1:nrow(sls), function(x) {
  
  bd$Time.expansion[bd$nombre.seleccion == gsub("\\.txt$", "", sls$selec.file[x])][1]
})

#sls$correg.caja[is.na(sls$correg.caja)] <- "vacios"

write.csv(sls, "C:/Users/Nazareth/Documents/Datos acusticos/selecciones ene2020.csv", row.names = FALSE)


mar<- mean(sls)

#para hacer spectros, revisar xq no abre archivo linea de cargar bd la puse yo 

#sls <- read.csv("C:/Users/Nazareth/Documents/Datos acusticos/selecciones nov2019.csv", stringsAsFactors = FALSE)

#espectros 
specreator(sls, ovlp = 90, fast.spec =F, dest.path = "C:/Users/Nazareth/Documents/Datos acusticos/spectros/", mar = 0.001, collevels = seq(-100, 0, 5), wl = 123)
...
sls <- read.csv("C:/Users/Nazareth/Documents/Datos acusticos/selecciones sep2019.csv", stringsAsFactors = FALSE)



table(sls$correg.caja, useNA = "ifany")

sls.si <- sls[sls$correg.caja == "Si", ]
sls.rev <- sls[sls$correg.caja == "Rev", ]
sls.vacios <- sls[sls$correg.caja == "vacios", ]


st <- sel_tailor(X = sls.rev, ovlp = 70, osci = FALSE,mar = 0.004, path = "C:/Users/Nazareth/Documents/Datos acusticos/",  flim = c(1, 193), wl = 123, collevels = seq(-100, 0, 5), auto.next = TRUE, pause = 0)

unlink("C:/Users/Nazareth/Documents/Datos acusticos/seltailor_output.csv")
